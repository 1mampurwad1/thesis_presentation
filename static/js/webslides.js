...
var Navigation = function () {
  ...
  _createClass(Navigation, [{
    ...
    value: function onSlideChanged_(event) {
      ...
      console.log(1)
      if (map) {
        var topLeftmap = L.latLng(52.223958730491624, 6.886185818666876);
        var bottomRightmap = L.latLng(-27.500487076806465, 153.01430728697466);
        var boundsmap = L.latLngBounds(topLeftmap, bottomRightmap);
        map.invalidateSize()
        map.fitBounds(boundsmap)
      } 
      if (bangkamap) {
        var topLeftbangkamap = L.latLng(-1.036132660635567, 104.37350177592182);
        var bottomRightbangkamap = L.latLng(-3.3401580092437957, 107.06235830343265);
        var boundsbangkamap = L.latLngBounds(topLeftbangkamap, bottomRightbangkamap);
        bangkamap.invalidateSize()
        bangkamap.fitBounds(boundsbangkamap)
      } 

      if(hyperaccumulatorMap){
        var topLefthyperaccumulatorMap = L.latLng(-10.455462016910062, 137.51698255867112);
        var bottomRighthyperaccumulatorMap = L.latLng(-28.98946380911413, 154.88357992191283);
        var boundshyperaccumulatorMap = L.latLngBounds(topLefthyperaccumulatorMap, bottomRighthyperaccumulatorMap);
        hyperaccumulatorMap.invalidateSize()
        hyperaccumulatorMap.fitBounds(boundshyperaccumulatorMap);
      }
